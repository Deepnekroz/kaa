...
[ "$JDBC_HOST" ] && sed -i "s/\(jdbc_host_port *= *\).*/\1$JDBC_HOST\:$JDBC_PORT/" /usr/lib/kaa-node/conf/sql-dao.properties;
[ "$JDBC_HOST" ] && sed -i "s/\(jdbc_url *= *\).*/\1jdbc\:mysql\:failover\:\/\/$JDBC_HOST\:3306\/kaa/" /usr/lib/kaa-node/conf/admin-dao.properties;
